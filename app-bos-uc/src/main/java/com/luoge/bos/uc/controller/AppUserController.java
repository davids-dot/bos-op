...
import com.luoge.bos.uc.service.AppUserService;
 ...
 */
@RestController
@RequestMapping("/bos-uc/setting/app")
public class AppUserController {
    ...
    public R<List<UserVO>> listUsers(String name, Context ctx) {
        return R.success(appUserService.listUser(ctx.getOrgId(), name));
    }
    ...
    public R<Void> insert(@RequestBody @Valid UserCreateBO user, Context ctx) {
        ...
        return appUserService.insert(ctx.getOrgId(), user);
    }
    ...
    public R<Void> update(@RequestBody @Valid UserUpdateBO user, Context ctx) {
        return appUserService.update(ctx.getOrgId(), user);
    }
    ...
    public R<Void> resetPasswd(@RequestBody @Valid UserPasswdBO reset, Context ctx) {
        ...
        appUserService.resetPasswd(ctx.getOrgId(), reset.getUserId(), reset.getPasswd());
        return R.SUCCESS;
    }

    @PutMapping("users/status")
    public R<Void> updateStatus(@RequestBody @Valid UserStatusBO userStatusBO, Context ctx) {
        appUserService.updateStatus(ctx.getOrgId(), userStatusBO.getUserId(), userStatusBO.getStatus());
        return R.SUCCESS;
    }
    ...
    public R<List<String>> listUserRoles(Integer userId, Context ctx) {
        var r = appUserService.listUserRoles(ctx.getOrgId(), userId);
    ...
    public R<Void> setUserRoles(@RequestBody @Valid UserRoleBO userRoleBO, Context ctx) {
        appUserService.setUserRoles(ctx.getOrgId(), userRoleBO.getUserId(), userRoleBO.getRoleIds());
        return R.SUCCESS;
    }
}
